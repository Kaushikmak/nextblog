...
export default function DashboardPage() {
    ...
    const handleSaveProfile = async () => {
        setIsSaving(true);
        try {
            // 1. Update Core Auth Record (Better Auth)
            if (formData.name !== session?.user?.name) {
                await authClient.updateUser({ name: formData.name });
            }

            // 2. Update Extended Profile Record (Convex)
            await upsertProfile({
    ...
    return (
        <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8 animate-in fade-in duration-500">
            <div className="flex items-center gap-4 mb-8">
                <div className="size-16 bg-primary/10 rounded-full flex items-center justify-center border">
